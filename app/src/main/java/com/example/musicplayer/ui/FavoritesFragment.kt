...
class FavoritesFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        songList = (activity as MainActivity).getFavorites()
        val lv: ListView = root.findViewById(R.id.list_songs)
        listViewAdapter = SongAdapter(requireActivity(), (activity as MainActivity).getFavorites())
        ...
        lv.setOnItemLongClickListener { parent, view, position, id ->
            ...
            popup.menuInflater.inflate(R.menu.favorites_popup_menu, popup.menu)
            popup.setOnMenuItemClickListener {
                val itemIdAtPos = parent.getItemIdAtPosition(position)
                val favSongPosition = itemIdAtPos.toInt()
                newList.remove(songList!![favSongPosition])
                (activity as MainActivity).removeFromFavorites(songList!![favSongPosition])
                songList = (activity as MainActivity).getFavorites()
    ...
    private fun initializeSongControl() {
        ...
        buttonNext.setOnClickListener {
            ...
            if (SystemClock.elapsedRealtime() - lastClickTime < 400) {
                ...
                toast.setText("There are no songs in your favorites.")
                toast.show()
            }
            else if(isShuffle){
                playNextSong(randomInt())
